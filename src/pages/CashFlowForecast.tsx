...
function StatusPill({ status }: { status: ForecastWeek["status"] }) { const map = { ok: { label: "Ok ✓", cls: "bg-[#ECFDF5] text-[#059669]" },
    ...
    recovery: { label: "Återhämtning ↑", cls: "bg-[#EFF6FF] text-[#2563EB]" },
  };
  const s = map[status];
          ...
          <div className="rounded-[10px] border border-destructive/20 bg-destructive/[0.04] overflow-hidden" style={{ borderLeft: "3px solid #DC2626" }}>
            ...
            {showAlertDetails && (
              <div className="px-4 pb-3 space-y-2 border-t border-destructive/10">
                ...
                <div className="flex gap-1.5 flex-wrap pt-2">
                  {weeks.map(w => (
                    <button key={w.weekIdx} className="text-[10px] px-2 py-0.5 rounded-full border" style={{ backgroundColor: w.status === "deficit" ? "#FEF2F2" : w.status === "low" ? "#FFFBEB" : w.status === "recovery" ? "#EFF6FF" : "#ECFDF5",
                      ...
                      color: w.status === "deficit" ? "#DC2626" : w.status === "low" ? "#D97706" : w.status === "recovery" ? "#2563EB" : "#059669",
                ...
                {/* Alert items */}
                {alerts.map(alert => (
                  <div key={alert.id} className="flex items-start gap-3 p-3 rounded-lg bg-card border" style={{ borderLeft: `3px solid ${alert.severity === "critical" ? "#DC2626" : alert.severity === "warning" ? "#F59E0B" : alert.severity === "opportunity" ? "#2563EB" : "#6B7280"}`,
                  }}>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <ChartGradients />
                    ...
                    <Line type="monotone" dataKey="closing" name="Kassasaldo" stroke="#2563EB" strokeWidth={2.5} dot={{ r: 3, fill: "#2563EB" }} />
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 shadow-[0_2px_16px_rgba(0,0,0,0.06)] p-6 h-[350px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <ComposedChart data={monthlyData.forecast}>
                          ...
                          <Line type="monotone" dataKey="base" name="Basfall" stroke="#2563EB" strokeWidth={2.5} dot={{ r: 3, fill: "#2563EB" }} {...LINE_ANIMATION} />
