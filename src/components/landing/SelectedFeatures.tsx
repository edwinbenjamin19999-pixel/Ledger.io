      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-12 grid grid-cols-1 gap-5 md:grid-cols-2">
          {features.map(({ icon: Icon, title, desc }) => (
            ...
            >
              <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-md bg-[#0F1B2D] text-white transition-transform duration-200 group-hover:scale-110">
                <Icon className="h-5 w-5" strokeWidth={2.5} aria-hidden />
              </div>
              <div>
                <h3 className="mb-1.5 text-lg font-bold text-[#0F1B2D]">{title}</h3>
                <p className="max-w-md text-sm leading-relaxed text-[#0F1B2D]/60">{desc}</p>
              </div>
            </div>
          ))}
        </div>
