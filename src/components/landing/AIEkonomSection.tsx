...
export const AIEkonomSection = () => {
  ...
  return (
    <section id="ai-ekonom" className="relative overflow-hidden bg-[#F5F8FF] py-24 px-6 md:py-28">
      <div
        aria-hidden
        className="pointer-events-none absolute -right-[6%] top-0 h-[460px] w-[460px] rounded-full bg-[#0052FF]/[0.06] blur-[130px]"
      />
      <div className="relative mx-auto grid max-w-6xl grid-cols-1 items-center gap-14 lg:grid-cols-2 lg:gap-16">
        ...
        <div>
          ...
          <h2 className="mt-5 font-display text-3xl font-bold leading-tight tracking-tight text-[#0F172A] md:text-[2.6rem]">
          ...
          <ul className="mt-8 flex flex-col gap-3.5">
            {POINTS.map((p) => (
              <li key={p} className="flex items-center gap-3">
                <span className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-[#0052FF] text-white">
                  <Check className="h-3.5 w-3.5" strokeWidth={3} aria-hidden />
                </span>
                <span className="text-[14.5px] text-[#0F172A]/80">{p}</span>
              </li>
            ))}
          </ul>
          ...
          <button
            ...
            className="group mt-9 inline-flex h-12 items-center gap-2 rounded-xl bg-[#0052FF] px-7 text-[15px] font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#0040CC] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0052FF] focus-visible:ring-offset-2 focus-visible:ring-offset-[#F5F8FF]"
          >
        ...
        {/* Chatt-mockup */}
        <div className="rounded-2xl border border-[#E2E8F0] bg-white p-4 shadow-[0_30px_60px_-30px_rgba(15,23,42,0.25)]">
