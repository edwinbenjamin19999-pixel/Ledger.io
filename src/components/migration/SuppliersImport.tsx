...
interface SuppliersImportProps { companyId: string;
...
export function SuppliersImport({ companyId, onImportComplete }: SuppliersImportProps) { const [open, setOpen] = useState(false);
  ...
  const downloadTemplate = () => { const template = [
      { 'Leverantörsnamn': 'Leverantör AB',
        'Organisationsnummer': '5569987654',
        'E-post': 'faktura@leverantör.se',
        'Telefon': '0819876543',
        'Adress': 'Leverantörsvägen 10, 222 33 Stockholm',
        'Kontaktperson': 'Bengt Bengtsson',
        'Betalningsvillkor (dagar)': '30'
      }
    ];

    const csv = generateCSV(template);
    downloadCSV(csv, "mall_leverantörer.csv");
    
    toast({ title: "Mall nedladdad",
      description: "CSV-mallen har laddats ner. Fyll i leverantörsuppgifter.",
    });
  };

  const validateRow = (row: Record<string, string>): ImportRow => { const errors: string[] = [];
    
    if (!row['Leverantörsnamn']) errors.push('Leverantörsnamn saknas');

    return { name: row['Leverantörsnamn'] || '',
  ...
  const handleImport = async () => { const validRows = previewData.filter(row => row.valid);
    ...
    try { const { data: { user } } = await supabase.auth.getUser();
      ...
      toast({ title: "Import genomförd!",
        description: `${validRows.length} leverantörer redo att importeras`,
      });
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Importera leverantörer från CSV</DialogTitle>
            <DialogDescription>
              Ladda upp en CSV-fil med leverantörsuppgifter från Fortnox, Visma eller annat system.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6">
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                <Label htmlFor="suppliers-file-upload" className="cursor-pointer">
              ...
              <>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>Leverantörsnamn</TableHead>
                ...
                <div className="flex justify-between items-center pt-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {importing ? "Importerar..." : `Importera ${previewData.filter(r => r.valid).length} leverantörer`}
                    </Button>
                  </div>
                </div>
              </>
