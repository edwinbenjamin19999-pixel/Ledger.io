  ...
  <section className="bg-white py-24 md:py-32">
    <div className="container mx-auto max-w-6xl px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {stats.map((s) => (
          <div key={s.label} className="rounded-xl border border-[#E2E8F0] bg-white p-7 text-center">
            <div className="text-4xl font-bold text-[#0052FF]" style={{ letterSpacing: "-0.5px" }}>{s.value}</div>
            <div className="mt-3 text-[#0F172A] font-semibold text-[14px]">{s.label}</div>
            <p className="mt-2 text-[13px] text-slate-500 leading-relaxed">{s.desc}</p>
          </div>
        ))}
      </div>
    </div>
