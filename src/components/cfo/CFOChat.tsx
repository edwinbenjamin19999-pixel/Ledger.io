...
const CFO_SYSTEM_PROMPT = `Du är Cogniq CFO — en senior finansiell rådgivare specialiserad på svenska småföretag. Du har djup kunskap om:
...
- Bolagsskatt: 20,6%`;
  ...
  const exportPDF = async () => { if (messages.length === 0) return;
    ...
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text("Cogniq CFO — Konversation", 14, 20);
    doc.setFontSize(9);
