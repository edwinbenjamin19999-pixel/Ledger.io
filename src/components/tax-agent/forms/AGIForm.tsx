      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Button onClick={runAIPrepare} className="bg-[#0891B2] hover:bg-[#0891B2]/90 text-white">
        ...
        <CardContent className="py-12">
          <div className="max-w-md mx-auto text-center space-y-6">
            <div className="relative w-20 h-20 mx-auto">
              <div className="absolute inset-0 rounded-full border-4 border-[#0891B2]/20" />
              <div className="absolute inset-0 rounded-full border-4 border-[#0891B2] border-t-transparent animate-spin" />
              <Bot className="absolute inset-0 m-auto h-8 w-8 text-[#0891B2]" />
            </div>
            <div>
            ...
            <div className="flex flex-col gap-1.5 text-left">
              {PREPARE_STEPS.slice(0, prepareStep + 1).map((s, i) => (
                <div key={i} className="flex items-center gap-2 text-xs">
                  {i < prepareStep ? <CheckCircle className="h-3.5 w-3.5 text-[#085041]" /> : <Loader2 className="h-3.5 w-3.5 animate-spin text-[#0891B2]" />}
  ...
  if (step === "submitting") { return (
      <Card>
        <CardContent className="py-16 text-center space-y-4">
          <Shield className="h-12 w-12 mx-auto text-[#0891B2] animate-pulse" />
        ...
        return (
          <div className="flex items-start gap-3 rounded-[12px]" style={{ background: "#FAEEDA", border: "0.5px solid #EF9F27", padding: "12px 14px" }}>
            ...
            <div className="space-y-0">
              ...
              <a href="/hr" className="text-[#0B4F6C] text-[11px] font-medium underline cursor-pointer mt-[4px] inline-block">Gå till personalregistret →</a>
            </div>
          </div>
        );
      })()}
                ...
                {warnings.map((w, i) => (
                  <div key={i} className="flex items-start gap-[8px] py-[5px]">
                    <div className={cn(
                      ...
                      w.severity === "error" ? "bg-[#E24B4A]" : w.severity === "warning" ? "bg-[#EF9F27]" : "bg-[#0B4F6C]"
                    )} />
              ...
              <tbody>
                {sortedEmployees.map((emp, idx) => {
                  ...
                  const rateChip = age === null ? (
                    ...
                    <Badge variant="outline" className="text-[10px] bg-[#0891B2]/10 text-[#0891B2] border-[#0891B2]/30">10,21% <span className="opacity-70 ml-0.5">(ung)</span></Badge>
                  ) : age > 65 ? (
                      ...
                      <CollapsibleTrigger asChild>
                        <tr className={cn(
                          ...
                          <td className="px-4 py-3">
                            <div className="w-9 h-9 rounded-full bg-[#0891B2]/10 text-[#0891B2] flex items-center justify-center text-xs font-bold">
                              {initials}
                            </div>
                          </td>
            ...
            {Object.entries(EMPLOYER_FIELD_LABELS).map(([code, label]) => { const val = employerFields[code] || 0;
              ...
              return (
                <div key={code} className="flex items-center justify-between py-2 border-b border-border/30">
                  ...
                  <div className="flex items-center gap-2">
                    {isImportant ? (
                      <span className="text-sm font-bold text-[#0891B2]">{fmt(val)} kr</span>
                    ) : (
                      <button onClick={() => { /* inline edit */ }}
                        className="text-sm font-medium text-right hover:text-[#0891B2] hover:underline cursor-pointer transition-colors flex items-center gap-1 group">
                        {fmt(val)} kr
      ...
      <div className="fixed bottom-[60px] md:bottom-0 left-0 right-0 z-40 bg-card/95 backdrop-blur-sm border-t border-border shadow-lg">
        <div className="max-w-screen-xl mx-auto px-4 py-3 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="text-sm font-bold">
            Totalt att betala: <span className="text-[#0B4F6C]">{fmt(totalToPay)} kr</span>
          </div>
          <div className="flex items-center gap-2">
            <Button
              size="sm"
            ...
            <DemoSubmitButton
              ...
              size="sm"
              disabled={!canSubmit}
              className="bg-[#0B4F6C] hover:bg-[#1074A0] text-[#E6F4FA] rounded-[8px] text-[12px] font-medium px-[16px] h-[36px] border-0"
          ...
          <div className="space-y-3 py-4">
            <Card className="bg-muted/30">
              <CardContent className="pt-4 pb-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">Totalt belopp</span><span className="font-bold text-[#0891B2]">{fmt(totalToPay)} kr</span></div>
