/**
 * Server-side variant of src/lib/skatteagent/bookSKVPayment.ts
 * Uses a service-role Supabase client passed in by the caller.
 *
...
export async function bookSKVPayment(
  ...
  const debitAccountId = await ensureAccount(supabase, companyId, debitSpec);
  const bankAccountId  = await ensureAccount(supabase, companyId, bankSpec);
  ...
  return {
    ...
    journalNumber: (je as any).journal_number ?? null,
    amount,
