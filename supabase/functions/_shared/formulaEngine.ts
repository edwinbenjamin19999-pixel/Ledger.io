/**
 * Deno port of src/lib/reports/formulaEngine.ts.
 * Safe formula evaluator for report rows. Supports [row.code] refs, +, -, *, /,
 * parens, unary minus. No eval/Function — recursive-descent parser.
 */
...
function parse(tokens: Token[], formula: string, values: Map<string, number>): number {
  ...
  function parseFactor(): number {
    ...
    if (t.type === "op" && t.op === "-") { consume(); return -parseFactor(); }
    if (t.type === "op" && t.op === "+") { consume(); return parseFactor(); }
...
export function resolveFormulas(
  ...
  function resolve(code: string): number {
    ...
    if (!formula) { out.set(code, 0); return 0; }
