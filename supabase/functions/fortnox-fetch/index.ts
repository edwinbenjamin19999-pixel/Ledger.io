/**
 * fortnox-fetch
 * Fetches customers, suppliers, customer invoices, supplier invoices from Fortnox
 * for the given company and inserts into imported_* tables.
 * Refreshes access token automatically when expired.
 */
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.45.0";
import { FortnoxFetcher } from "./fortnoxFetcher.ts";
...
async function refreshIfNeeded(supabase: any, conn: any) {
  ...
  const r = await fetch(FORTNOX_TOKEN_URL, {
    method: "POST",
    headers: {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (connErr || !conn) {
      return new Response(
        JSON.stringify({ error: "Ingen aktiv Fortnox-anslutning" }),
        { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }

    const accessToken = await refreshIfNeeded(supabase, conn);
    const fetcher = new FortnoxFetcher(accessToken);
    ...
    const { data: job, error: jobErr } = await supabase
      ...
      .insert({
        ...
        source_format: "fortnox_api",
        source_filename: "Fortnox API",
        status: "fetching",
    ...
    try {
      ...
      const invoices = await fetcher.fetchInvoices(fromDate, undefined, (n) => (stats.customerInvoices = n));
    ...
    console.error("fortnox-fetch error:", e);
    return new Response(
