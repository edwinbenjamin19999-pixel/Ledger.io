...
BEGIN
  IF NOT (
    ...
    RETURN QUERY SELECT
      ...
      0::numeric, 0::numeric, 0::integer, 0::numeric, 0::numeric,
      NULL::date, 0::numeric, 0::integer, 0::numeric, 0::integer,
      0::numeric, 0::integer, 0::integer, NULL::date, 0::numeric
    WHERE false;
    RETURN;
  END IF;
  ...
  WITH ledger AS (
    ...
    SELECT
      COALESCE(SUM(credit - debit) FILTER (WHERE account_number >= '3000' AND account_number < '4000' AND entry_date BETWEEN _month_start AND _month_end), 0)::numeric AS revenue_mtd,
      COALESCE(SUM(debit - credit) FILTER (WHERE account_number >= '4000' AND account_number < '8000' AND entry_date BETWEEN _month_start AND _month_end), 0)::numeric AS costs_mtd,
      COALESCE(SUM(debit - credit) FILTER (WHERE account_number >= '1910' AND account_number < '1950' AND entry_date <= _month_end), 0)::numeric AS cash_balance,
      COALESCE(SUM(debit - credit) FILTER (WHERE account_number >= '1500' AND account_number < '1600' AND entry_date <= _month_end), 0)::numeric AS ar_balance,
      COALESCE(SUM(credit - debit) FILTER (WHERE account_number >= '2440' AND account_number < '2450' AND entry_date <= _month_end), 0)::numeric AS ap_balance,
      COALESCE(SUM(credit - debit) FILTER (WHERE account_number >= '2610' AND account_number < '2640' AND entry_date BETWEEN _month_start AND _month_end), 0)::numeric AS output_vat,
      COALESCE(SUM(debit - credit) FILTER (WHERE account_number >= '2640' AND account_number < '2650' AND entry_date BETWEEN _month_start AND _month_end), 0)::numeric AS input_vat,
      COALESCE(SUM(credit - debit) FILTER (WHERE account_number >= '3000' AND account_number < '4000' AND entry_date BETWEEN _year_start AND current_date), 0)::numeric AS revenue_12m,
    ...
    SELECT CASE
      WHEN period_month IS NOT NULL THEN (make_date(period_year, period_month, 1) + interval '1 month - 1 day')::date
      WHEN period_quarter IS NOT NULL THEN (make_date(period_year, period_quarter * 3, 1) + interval '1 month - 1 day')::date
      ELSE make_date(period_year, 12, 31)::date
    END AS period_end
